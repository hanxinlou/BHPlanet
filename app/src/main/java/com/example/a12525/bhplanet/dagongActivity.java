...
import static com.example.a12525.bhplanet.wangjingzeActivity.JSON;
...
public class dagongActivity extends AppCompatActivity {
    ...
    private Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            if(msg.what==0x123) {
                Intent intent = new Intent(dagongActivity.this,wangjingzewActivity.class);
                intent.putExtra("gif_url", "https://sorry.xuty.tk" + resData);
                Toast.makeText(dagongActivity.this, "图片生成中……", Toast.LENGTH_LONG).show();
    ...
    public void getDatasync(){
        new Thread(() -> {
            try {
                String url = "https://sorry.xuty.tk/api/dagong/make";
                String json = getJson();
                ...
                if (response.isSuccessful()) {
                    Log.d("dagong","response.code()=="+response.code());
                    Log.d("dagong","response.message()=="+response.message());
                    resData = response.body().string();
                    Log.d("dagong","res=="+resData);
